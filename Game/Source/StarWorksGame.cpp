        ...
        [[nodiscard]] sw::MeshData buildStarfieldMesh()
        {
            ...
            for (sw::u32 star = 0; star < kStarCount; ++star)
            {
                ...
                if (inBand)
                {
                    ...
                    // TWO STATEMENTS, DELIBERATELY. Written as
                    // `hash01(seed++) + hash01(seed++)` the two increments of
                    // `seed` are unsequenced relative to each other: the
                    // standard leaves the order — and therefore which seed
                    // each call gets — up to the compiler, so MSVC and GCC
                    // are free to generate two different skies from the same
                    // save. The sum of two uniforms is wanted here (it is
                    // what makes the band's cross-section gaussian-ish);
                    // being unable to say which two is not.
