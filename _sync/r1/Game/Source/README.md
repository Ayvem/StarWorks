...
| `GameSaveLoad.cpp` | The save schema (every component the world may hold), quicksave, quickload. **A component added to the world must be registered here** — `Snapshot` throws on save otherwise, by design. |
...
2. **A helper needed by two files moves to `GameInternal.hpp`** (inline),
   not copy-pasted. A helper needed by one file stays in that file's
   anonymous namespace (see the hangar's quaternion helpers).
