...
craft somewhere fast, because the cost of a flight plan depends on WHERE the arc
starts as much as on how quick it is. `SW_FRAMEPROBE` answers "which part of the
frame was slow" — a frame-rate counter averages over a second and cannot see a
single frame that took a sixth of one, which is what a player means by "it
freezes but it still says 200 fps".
